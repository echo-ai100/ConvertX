...
import { COUNTABLE_CONVERSION_JOB_SQL } from "../helpers/jobs";
import { userService } from "./user";
import { t as translate, detectLocale } from "../locales";

export const admin = new Elysia().use(userService).get(
  "/admin",
  async ({ user, redirect, headers, cookie: { locale } }) => {
    ...
    if (
      !userData ||
      (userData.role !== "admin" && !ADMIN_EMAILS.includes(userData.email.toLowerCase()))
    ) {
    ...
    const jobCount = db
      .query(`SELECT COUNT(*) as count FROM jobs WHERE ${COUNTABLE_CONVERSION_JOB_SQL}`)
      .get() as {
      count: number;
    } | null;
    const completedJobs = db
      .query(
        `SELECT COUNT(*) as count FROM jobs WHERE status = 'completed' AND ${COUNTABLE_CONVERSION_JOB_SQL}`,
      )
      .get() as { count: number } | null;
    const failedJobs = db
      .query(
        `SELECT COUNT(*) as count FROM jobs WHERE status = 'failed' AND ${COUNTABLE_CONVERSION_JOB_SQL}`,
      )
      .get() as { count: number } | null;
    const creditConsumed = db
      .query(
              ...
              <section class="mb-6">
                ...
                <div class="grid grid-cols-2 gap-4 sm:grid-cols-3">
                  ...
                  <div class="p-4 bg-neutral-800 rounded-sm">
                    ...
                    <p class="text-lg">
                      {(cpuLoad[0] ?? 0).toFixed(2)} / {(cpuLoad[1] ?? 0).toFixed(2)} /{" "}
                      {(cpuLoad[2] ?? 0).toFixed(2)}
                    </p>
                  </div>
                  <div class="p-4 bg-neutral-800 rounded-sm">
