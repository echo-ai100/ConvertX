...
export function calculateFileCredits(fileSizeBytes: number, normalizedFileType: string): number {
...
export function calculateJobCredits(fileNames: string[], userUploadsDir: string): number {
  let total = 0;
...
export function deductCredits(
  ...
  const result = db
    .query("UPDATE users SET credits = credits - ? WHERE id = ? AND credits >= ?")
    .run(amount, userId, amount);
  if (result.changes === 0) return false;
  db.query(
    "INSERT INTO credit_transactions (user_id, amount, type, reference_id, description, date_created) VALUES (?, ?, ?, ?, ?, ?)",
  ).run(userId, -amount, "conversion", jobId, `Conversion job #${jobId}`, new Date().toISOString());
  return true;
}

export function refundCredits(userId: string | number, amount: number, jobId: number | null): void {
  db.query("UPDATE users SET credits = credits + ? WHERE id = ?").run(amount, userId);
  db.query(
    "INSERT INTO credit_transactions (user_id, amount, type, reference_id, description, date_created) VALUES (?, ?, ?, ?, ?, ?)",
...
export function getUserCredits(userId: string | number): number {
  const user = db.query("SELECT credits FROM users WHERE id = ?").get(userId) as {
    credits: number;
  } | null;
